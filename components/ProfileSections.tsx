...
export function ListSection({ items, lang = 'ko' }: { items: { title: string, subtitle: string, date: string, link?: string, category?: string, status?: string, idNumber?: string, expiryDate?: string }[], lang?: 'ko' | 'en' }) {
  return (
      ...
      {items.map((item, i) => (
        <li key={i} className="border-l-2 border-[var(--border)] pl-4 py-1">
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            {(item.idNumber || item.expiryDate) && (
              <p className="text-xs text-[var(--text-muted)] flex flex-wrap gap-x-4 gap-y-1">
                {item.idNumber && (
                  <span>
                    <span className="font-semibold uppercase tracking-wider mr-1 opacity-70">ID</span> 
                    {item.idNumber}
                  </span>
                )}
                {item.expiryDate && (
                  <span>
                    <span className="font-semibold uppercase tracking-wider mr-1 opacity-70">{lang === 'ko' ? '만료일' : 'Exp'}</span> 
                    {item.expiryDate}
                  </span>
                )}
              </p>
...
const getPrizeColor = (prize: string) => {
  ...
  if (lowerPrize.includes('금상') || lowerPrize.includes('gold') || lowerPrize.includes('최우수') || lowerPrize.includes('top excellence'))
  ...
  // 은상 (Silver) = 우수상
  if (lowerPrize.includes('은상') || lowerPrize.includes('silver') || lowerPrize.includes('우수') || lowerPrize.includes('excellence'))
